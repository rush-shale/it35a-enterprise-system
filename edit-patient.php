...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    // Handle the form submission to update the patient
    ...
    try {
        $stmt = $conn->prepare("UPDATE patients SET full_name = ?, birth_date = ?, gender = ?, phone = ?, email = ? WHERE patient_id = ?");
        $stmt->execute([$full_name, $birth_date, $gender, $phone, $email, $patient_id]);
...
<head>
    <meta charset="UTF-8">
    <title>Edit Patient - Medicare Admin</title>
    <style>
        body {
...
<body>
    ...
    <?php if (isset($_GET['updated']) && $_GET['updated'] == 1): ?>
        <div class="message">Patient record updated successfully.</div>
    <?php endif; ?>
